...
locals {
  ...
  base_image          = "${data.aws_caller_identity.this.account_id}.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/${var.env}-isengardecr:${local.image_version}"
  node_env = lookup({
    "dev"   = "development",
    "stage" = "stage",
    "prod"  = "production"
  ...
  base_ecr_url = "${local.ecr_address}/${data.terraform_remote_state.platform-infra.outputs.dev-isengardecr}"
}
